...
CONFIG_AND_HELP = {
    ...
    "maximum-matrix-memory-cache-size": _(
        "500MB",
        """The maximum memory size of the in-memory matrix cache in bytes. Only used when
        ``matrix-memory-cache-policy`` is ``"largest"`` or ``"lru"``. Can be set to None.
    ...
    "matrix-memory-cache-strict-mode": _(
        False,
        """Raise exception if the matrix cannot be fitted into the in-memory cache.
        Only used when ``matrix-memory-cache-policy`` is ``"largest"`` or ``"lru"``.
        See :ref:`memory_cache` for more information.""",
    ),
}
