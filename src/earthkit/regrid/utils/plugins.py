...
from earthkit.regrid.utils.humanize import did_you_mean
...
def _load_plugins(kind):
    plugins = {}
    for e in entrypoints.get_group_all(f"earthkit.regrid.{kind}s"):
...
def directories(owner: bool = False) -> list:
    ...
    result.append(("earthkit.regrid", os.path.dirname(earthkit.regrid.__file__)))
