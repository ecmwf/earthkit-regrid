...
def test_regrid_matrix_method_kwarg(_kwarg, method):
...
def _ll_to_ll(method):
    ...
    v_res, res_grid = regrid(
        v_in, {"grid": [5, 5]}, out_grid=out_grid, method=method, backend="system-matrix"
    )
...
def test_regrid_matrix_ll_to_ll(method):
    _ll_to_ll(method)


@pytest.mark.download
@pytest.mark.parametrize("method", METHODS)
def test_regrid_matrix_ll_to_ll_user_cache(method):
    _ll_to_ll(method)
...
@pytest.mark.tmp_cache
@pytest.mark.parametrize("method", METHODS)
def test_regrid_matrix_ogg_to_ll(method):
    f_in, f_out = get_test_data(["in_O32.npz", f"out_O32_10x10_{method}.npz"])
...
@pytest.mark.tmp_cache
@pytest.mark.parametrize("method", METHODS)
def test_regrid_matrix_ngg_to_ll(method):
    f_in, f_out = get_test_data(["in_N32.npz", f"out_N32_10x10_{method}.npz"])

    v_in = np.load(f_in)["arr_0"]
    v_ref = np.load(f_out)["arr_0"]
    out_grid = {"grid": [10, 10]}
    v_res, grid_res = regrid(v_in, {"grid": "N32"}, out_grid=out_grid, method=method, backend="system-matrix")
...
@pytest.mark.tmp_cache
@pytest.mark.parametrize("method", METHODS)
def test_regrid_matrix_healpix_ring_to_ll(method):
    ...
    out_grid = {"grid": [10, 10]}
    v_res, grid_res = regrid(
        v_in, {"grid": "H4", "ordering": "ring"}, out_grid=out_grid, method=method, backend="system-matrix"
    )
...
@pytest.mark.tmp_cache
@pytest.mark.parametrize("method", METHODS)
def test_regrid_matrix_healpix_nested_to_ll(method):
    ...
    out_grid = {"grid": [10, 10]}
    v_res, grid_res = regrid(
        v_in, {"grid": "H4", "ordering": "nested"}, out_grid=out_grid, method=method, backend="system-matrix"
    )
...
def test_regrid_matrix_unsupported_input_grid() -> None:
    ...
    with pytest.raises(ValueError):
        _ = regrid(a, {"grid": [2.2333424, 2]}, {"grid": [1, 1]}, method="linear", backend="system-matrix")
...
def test_regrid_matrix_unsupported_output_grid() -> None:
    a = np.ones(181 * 360)
    with pytest.raises(ValueError):
        _ = regrid(a, {"grid": [1.11323424, 1]}, {"grid": [5, 5]}, method="linear", backend="system-matrix")
