...
# NOTE: The tests are skipped for now as it is not yet decided if the weights
...
def test_local_memcache_core_1(policy, adjust_to, evict):
    ...
    with config.temporary():
        ...
        # We rely on the first weights being larger than the second one.
        assert mem_first > mem_second

        # reduce max size so that the weights specified by "evict" should be evicted.
        if adjust_to == "first":
...
def test_local_memcache_core_2(policy, adjust_to, evict):
    ...
    with config.temporary():
        ...
        # We rely on the first weights being smaller than the second one.
