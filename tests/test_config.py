...
from earthkit.regrid.utils.temporary import temp_file


def read_config_yaml(path=os.path.expanduser("~/.config/earthkit/regrid/config.yaml")):
    try:
        with open(path) as f:
            import yaml
...
@pytest.mark.parametrize(
    ...
    [
        ("url-download-timeout", "A", "A", ValueError),
...
def test_config_env(monkeypatch, value, error):
    env_key = "EARTHKIT_REGRID_URL_DOWNLOAD_TIMEOUT"
